/*
  # Add embedding jobs table and policies
...
DO $$ 
BEGIN
  -- Create policies for embedding_jobs table if they don't exist
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'embedding_jobs' 
    AND policyname = 'Users can insert their own embedding jobs'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'embedding_jobs' 
    AND policyname = 'Users can read their own embedding jobs'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'embedding_jobs' 
    AND policyname = 'Users can update their own embedding jobs'
  ) THEN
