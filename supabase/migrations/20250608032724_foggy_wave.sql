/*
  # Fix Agent Session Management

  1. Agent Table Policies
    - Recreate RLS policies for proper user isolation
    - Ensure authenticated users can manage their own agents

  2. Agent Management Functions
    - `create_agent_session()` - Create new agent session
    - `get_active_agent()` - Get user's active agent
    - `terminate_agent_session()` - Terminate agent session

  3. Security
    - All functions use SECURITY INVOKER with RLS
...
-- Add a function to create agent sessions with default parameter
...
-- Grant execute permission to authenticated users (only for the actual function signature)
